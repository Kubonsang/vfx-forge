...
{
    public sealed class ProductionCrescentContractTests
    {
        private const string Root =
            "Assets/VFXForge/Dogfood/ProductionCrescentSlash";
        ...
        [Test]
        public void Catalog_ProductionEntryBindsEveryRequiredRecipeProperty()
        {
            VfxTemplateCatalog catalog =
                AssetDatabase.LoadAssetAtPath<VfxTemplateCatalog>(
                    Root + "/ProductionCrescentCatalog.asset");

            Assert.That(catalog, Is.Not.Null);
            Assert.That(
                catalog.TryGet(
                    "production_crescent_slash_v1",
            ...
            Assert.That(entry.bindings.All(binding => binding.required), Is.True);
            Assert.That(
                entry.bindings.Select(binding => binding.exposedPropertyName),
                Is.EquivalentTo(RequiredProperties));
        }

        [Test]
        public void CompiledPrefabs_HaveDistinctOverridesAndProductionPolicies()
        {
            GameObject primary = LoadGenerated("ProductionCrescentSlash.prefab");
            GameObject variant = LoadGenerated("ProductionCrescentSlashVariant.prefab");
            ...
            Assert.That(variantEffect.visualEffectAsset, Is.SameAs(primaryEffect.visualEffectAsset));
            ...
            foreach (string property in RequiredProperties.Skip(1).Take(7)
            ...
            Assert.That(primary.GetComponentsInChildren<Light>(true), Is.Empty);
            Assert.That(
                primary.GetComponentsInChildren<MeshRenderer>(true)
                    .All(renderer => renderer.sharedMaterial.renderQueue >= 3000),
                Is.True);
            Assert.That(
            ...
            Assert.That(
                ...
                Is.EqualTo("production_crescent_slash"));
            Assert.That(
                variant.GetComponent<VfxMetadata>().recipeId,
                Is.EqualTo("production_crescent_slash_variant"));
        }
        ...
        private static void AssertRequiredProperties(VisualEffect effect)
        {
